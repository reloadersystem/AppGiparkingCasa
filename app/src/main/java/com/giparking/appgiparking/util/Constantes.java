package com.giparking.appgiparking.util;

import java.util.ArrayList;

/**
 * Created by jledesma on 10/7/19.
 */

public class Constantes {

    public static final String URL_BASE = "http://148.102.120.248/GiParkingServicioMovil/Sistema/";

    public static final String PREFERENCIA_USUARIO = "preferencia_usuario";
    public static final String Empresa = "empresa";

    public static final String MENU_CONTROL = "Control";
    public static final String MENU_CONTROL_MANUAL = "Control Manual";
    public static final String MENU_REIMPRIMIR_COMPROBANTE = "Reimprimir Comprobante";
    public static final String MENU_ANULACION = "Anulación";
    public static final String MENU_MOVIMIENTO = "Movimiento";
    public static final String MENU_CIERRE_CAJA = "Cierre de Caja";

}
